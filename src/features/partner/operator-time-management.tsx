...
export function OperatorDashboard() {
  ...
  return <PageShell>
    ...
    <div className="mt-6 grid gap-3"><Button as={Link} fullWidth href="/partner/slots/new">시간 등록하기</Button><Button as={Link} fullWidth href="/partner/slots" variant="secondary">시간 관리 보기</Button>{application.canPublish ? <Button as={Link} fullWidth href="/partner/court-photos" variant="secondary">대표 코트 사진 관리</Button> : null}</div>
  </PageShell>;
}
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (slotId) {
        ...
        setDraft({ courtUnitName: existing.court.courtNumber, date: start.date, startsAt: start.time, endsAt: end.time, priceKrw: String(existing.guestFeeKrw), maxParticipantCount: String(existing.maxParticipantCount), usageNote: existing.usageNote ?? "" });
      }
