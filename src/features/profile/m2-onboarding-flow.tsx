...
/** 입력 화면 수. 닉네임 + 질문 네 개다. 막대와 숫자가 같은 값을 보게 한다. */
const STEP_COUNT = 5;

function FormShell({ children, onBack, step }: { children: React.ReactNode; onBack: () => void; step: number }) {
  return <main className="min-h-svh bg-[var(--tm-bg-page)] px-5 py-6 text-[var(--tm-text-primary)]"><section className="mx-auto flex min-h-[calc(100svh-3rem)] max-w-[480px] flex-col"><header className="flex items-center gap-4"><button aria-label="이전 화면으로 돌아가기" className="grid size-11 place-items-center rounded-full" onClick={onBack} type="button">←</button><div className="flex-1"><div className="h-1 overflow-hidden rounded-full bg-[var(--tm-border-default)]"><span className="block h-full bg-[var(--tm-action-primary)]" style={{ width: `${(step / STEP_COUNT) * 100}%` }} /></div><p className="mt-1 text-right text-xs text-[var(--tm-text-secondary)]">{step}/{STEP_COUNT}</p></div></header><div className="pt-8 [&>h1]:text-2xl [&>h1]:font-bold [&>h1]:leading-snug [&>p]:mt-3 [&>p]:leading-6 [&>p]:text-[var(--tm-text-secondary)]">{children}</div></section></main>;
}
