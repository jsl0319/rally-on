...
export function SupportThread({ inquiry: i, audience, refresh, endpointOverride }: { endpointOverride?: string; inquiry: SupportInquiryView; audience: SupportAudience; refresh: () => Promise<void> }) {
  const [body, setBody] = useState("");
  const [action, setAction] = useState(audience === "operator" ? "OPERATOR_REPLY" : "REPLY");
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const pending = useRef<{ key: string; id: string } | null>(null);
  const endpoint = endpointOverride ?? (audience === "reviewer" ? "/api/internal/support-inquiries" : audience === "operator" ? "/api/v1/operator/support-inquiries" : "/api/v1/support-inquiries");
  ...
  return <article className="rounded-2xl border border-slate-200 bg-white p-4 text-sm">
    ...
    {audience === "reviewer" && !i.assignedToMe ? <Button className="mt-4" disabled={busy || (i.assigned && !i.canReassign)} size="medium" onClick={() => void send("CLAIM", i.canReassign ? "이전 담당자의 처리 권한이 종료되어 문의를 인계받았어요." : "담당자가 문의 검토를 시작했어요.")}>{i.canReassign ? "비활성 담당자의 문의 인계받기" : i.assigned ? "다른 담당자 검토 중" : "문의 담당하기"}</Button> : null}
