...
const include = {
  ...
  match: {
    select: {
      id: true, title: true, startsAt: true, hostUserId: true, courtSlotId: true,
...
function toView(i: Inquiry, audience: SupportAudience, viewerId: string) {
  ...
  return {
    ...
    assignedToMe: i.assigneeUserId === viewerId, assigned: Boolean(i.assigneeUserId),
...
export async function listMySupportInquiries(prisma: PrismaClient, userId: string) {
  const items = await prisma.supportInquiry.findMany({ where: { userId }, orderBy: [{ createdAt: "desc" }, { id: "desc" }], include });
...
export async function actOnSupportInquiry(prisma: PrismaClient, viewer: { id: string; role: string }, inquiryId: string, audience: SupportAudience, input: z.infer<typeof supportActionInputSchema>) {
  if (audience === "reviewer") assertInternalReviewer(viewer);
  return prisma.$transaction(async (tx) => {
    ...
    if (audience === "member") {
      ...
      if (input.action === "CLAIM") {
        if (inquiry.assigneeUserId && inquiry.assigneeUserId !== viewer.id) conflict("다른 담당자가 맡고 있어요.");
        status = "IN_PROGRESS";
      } else {
    ...
    await tx.supportInquiryMessage.create({ data: { inquiryId, authorUserId: viewer.id, clientRequestId: input.clientRequestId, action: input.action, visibility: ["REQUEST_OPERATOR", "OPERATOR_REPLY"].includes(input.action) ? "OPERATOR" : "PUBLIC", body: input.body } });
