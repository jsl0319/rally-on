...
이 문서는 Rally On Core MVP를 개인 개발자가 작은 단위로 구현하고 매 단계에서 직접 검증할 수 있도록 작업 순서, 의존성, 완료 기준과 테스트 전략을 정의한다.
...
- 코트 확보 게이트, 시작·종료 시간을 직접 선택하는 4단계 모바일 등록 화면, 코트명·주소 직접 입력을 구현했다.
- 로그인에 쓰는 카카오 REST API 키를 서버에서만 사용해 테니스장 장소 검색을 제안하고, 선택한 결과를 코트명·주소에 채우는 보조 흐름을 구현했다. 키·외부 조회 오류 때도 직접 입력은 계속 제공한다.
...
- `GET /api/v1/court-place-search` — 인증·온보딩 완료 사용자만, 최대 5개 이름·주소 결과만 반환
- `clientRequestId`와 모집자 복합 유일성 추가

### 11.2 화면

- S05 코트 확보 게이트
- 시작·종료 시간, 지역, 장소 검색·직접 코트 입력
- 플레이·모집 조건 입력
- 비용·안내 입력
...
- 장소 검색 결과는 예약·운영 상태를 검증하거나 저장하지 않으며, 카카오 원본 응답의 전화번호·좌표·장소 URL을 반환하지 않는다.
- 단계 이동 시 입력을 보존하지만 Core에서 서버 임시저장은 만들지 않는다.

### 11.4 테스트

- 과거 시작 시각과 시작 시간보다 빠르거나 같은 종료 시간
- 없는 Region
- 코트명·주소 누락
- 장소 검색의 한 글자 검색어, 온보딩 미완료 접근, 키·공급자 조회 실패와 직접 입력 폴백
- 모집 인원 0명
- 플레이 목적 0개·3개
- 음수 비용과 무료 코트
- `PARTNER_COURT` 생성 시도
...
- 외부에서 코트를 예약한 사용자가 모바일에서 시작·종료 시간과 코트를 입력해 매칭을 등록한다.
- 검색 결과를 선택하거나 직접 입력해도 동일한 코트명·주소 검증과 등록 흐름을 사용한다.
...
최소 두 개의 독립 계정으로 다음 두 흐름을 자동 또는 수동 검증한다.

1. 외부 예약 코트: 사용자 A가 코트명·주소·시작·종료 시간·전체 비용을 입력해 `EXTERNAL_RESERVED` Match를 등록하고, 사용자 B가 발견·신청·수락·채팅까지 진행한다.
2. 코트 매칭: 공개된 `AVAILABLE` CourtSlot을 사용자 A가 하나의 `PARTNER_COURT` Match에 연결하고, 사용자 B가 신청·수락·채팅까지 진행한다.

과거 `COURT_TBD` Match는 새 생성·공개 목록·추천·새 신청에서 제외되는지, 기존 모집자와 기존 수락 참가자의 상세·채팅·완료 처리는 유지되는지를 별도로 확인한다.
...
- 자동 시나리오는 390×844에서 다음을 검증한다: 새 일반 매칭의 외부 예약 코트 필수 입력과 `COURT_TBD` 선택 UI 부재, 외부 예약 코트 Match의 신청→수락→Match 채팅 및 제3자 접근 거부, 공개 CourtSlot의 `PARTNER_COURT` 세션 연결→신청→수락→채팅, 과거 `COURT_TBD`의 비공개·신규 신청 차단과 기존 참여자의 상세·채팅·완료 처리.
- 동일 Slot의 동시 연결과 정원 제한은 브라우저 지연에 의존하지 않고 실제 PostgreSQL을 쓰는 도메인·통합 테스트에서 함께 검증한다.
...
- 운영자가 코트 매칭의 주최자이며 참가 신청 승인·입금 확인도 운영자가 한다.
- 코트 매칭은 코트 매칭 탭에서만 보여 주고 매칭 상세에는 출처 배지를 두지 않는다.
...
2026-09-01 공개 문서 기반 PG 사전 검증은 `09-court-commerce-pg-preflight.md`에 기록했다. 토스페이먼츠는 결제·취소·셀러 지급·상태 웹훅 측면에서 1순위 조건부 후보이나, 계약상 판매자·업종·KYC·지급 보류/환수·수수료·세무 책임과 sandbox 한계는 아직 구현 차단 항목이다.
...
- 모집자 단일 결제, 시작 24시간 전 전액 취소, 최소 인원 미달, 운영자 공급 철회·우천, 참가자 현장 비용 분담 비보장, PG 장애의 환불·지원 정책이 문서화된다.
- 운영자·Rally On·PG의 수납·환불·지급·세무 책임과 개인정보 보관 방식이 법무·세무·PG 계약으로 확정된다.
...
2. 코트 이용 총액·요금표 버전, 시작 24시간 전 전액 취소, 최소 인원 미달, 현장 자율 비용 분담 비보장, 우천·노쇼, 고객 지원 문구를 사용자에게 보이는 정책까지 확정한다.
3. `OperatorCommerceAccount`, `CourtRateCardVersion`, `CourtSlotCommercePolicy`, 단일 결제 홀드·원장·정산 대사 모델을 별도 migration으로 구현한다.
4. 한 Slot의 단일 15분 결제 홀드, PG 서명 검증, 승인 시 `AVAILABLE → ALLOCATED`와 Match·모집자 결제 확정의 원자성을 먼저 검증한다.
5. 모집자 24시간 전 전액 취소, 최소 인원 미달의 진행·취소·무응답 처리, 운영자 공급 철회 전액 환불을 구현한다.
6. PG 지급 대사, webhook 실패·늦은 승인 경보와 운영자 정산 조회를 구현한다.
7. 참가자별 앱 결제·비용 수납 UI가 없는지 포함해 결제 UI를 모바일에서 검증하고, Figma 사용량 한도가 해제된 뒤 사용자가 명시적으로 요청할 때만 Figma에 동기화한다.
...
## 2026-09-07 개설 입력 개선 검증·적용 상태

- 일반 매칭: 제목 입력 제거, 게임 유형 6종·선택 정산 계좌·넓은 매칭 소개글 입력, 자동 소개 생성 제거. 목록·상세는 코트명 표시. 제휴 세션 개설의 제목 입력도 제거.
- 계좌는 모집자·ACCEPTED 참가자의 상세 응답에만 포함하고 응답 캐시는 private/no-store로 제한. 기존 데이터는 nullable 열 추가로 보존.
- 단위 테스트에 미수락자·제3자 계좌 비노출, 모집자·수락자 열람, 계좌 형식·부분 입력 거절, 멱등성 검증 추가.
- 마이그레이션 `20260907123000_match_game_and_settlement` 준비. 공유 Neon 적용은 자동 승인 검토에서 거절되어 명시적 대상 승인을 기다린다. 원격에만 있는 과거 이력 `20260902010000_add_partner_court_direct_transfer`는 별도 운영자 테이블과 관련되며 변경·삭제하지 않았다. 적용 후 Prisma 재생성·개발 서버 재시작·실제 화면 검증 필요.
- E2E_DATABASE_URL 미설정, 로컬 Docker/PostgreSQL 실행 도구 미발견으로 DB 기반 E2E는 미실행. 운영 DB를 E2E 초기화 대상으로 사용하지 않는다.
