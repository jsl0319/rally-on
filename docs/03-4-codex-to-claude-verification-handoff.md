...
> **2026-09-09 갱신:** 이 표는 인수인계 시점의 기록이다. 위 공백 가운데 실제
> PostgreSQL 동시성과 브라우저 E2E는 §12에서 메워졌다. 크론은 여전히 보류다.
...
## 12. 검증 결과 (2026-09-09, Claude)

§4~§7의 임무를 실행했다. 전용 DB를 준비하고 브라우저 E2E와 실제 PostgreSQL 동시성까지 확인했다.

### 12.1 실행 환경

Docker가 없는 맥이라 Homebrew PostgreSQL 17로 **전용 포트 55432에 전용 클러스터**를
따로 띄웠다(`~/.rally-on-e2e-pg`). 기본 포트 5432, `brew services` 자동 시작, 기존
`DATABASE_URL`(공유 Neon)은 건드리지 않았다. `.env.local`에는 `E2E_DATABASE_URL` 한
줄만 추가했다. 설치 절차는 README `브라우저 E2E`에 남겼다.
...
| --- | --- |
| 린트·타입 검사 | 통과 |
| Vitest 단위/API/DB 테스트 | **48개 파일, 286개 통과** (기존 269 + 신규 17) |
| 프로덕션 빌드 | 통과 |
| Playwright 브라우저 E2E | **6개 통과** (기존 5 + 신규 1) |
| 실제 PostgreSQL 동시성 | **4개 통과** — 별도 연결이 Match 행 잠금을 쥔 상태에서 검증 |
| 무접속 상태의 정시 취소 알림 | **여전히 미해결.** 크론 보류 유지 |

`tests/db/court-match-flow.test.ts`(신규, 17개)가 §6 권한·상태 경계와 §7 동시성을
덮는다. `E2E_DATABASE_URL`이 없으면 통째로 건너뛰므로, DB 없이 `npm run check`만
돌리면 실행되지 않는다.

동시성 4건은 `Promise.all`로 겹쳤다고 가정하지 않는다. 별도 연결이 `SELECT ... FOR
UPDATE`로 Match 행을 실제로 쥔 뒤 요청들을 시작시키고, 잠금을 놓은 다음 결과를 받는다.

- 마지막 한 자리에 동시 신청 → 한 명만 `ACCEPTED`, 나머지는 `NO_REMAINING_SPOTS`
- 마지막 자리를 두 신청에 동시 승인 → 한 건만 승인
- 환불 완료가 먼저 잠금을 얻음 → 뒤따르는 계좌 수정은 `REFUND_ALREADY_COMPLETED`, 기존 계좌 유지
- **잠금을 기다리는 동안 판정 시점을 지난 승인 요청** → 대기 전 시각으로 승인하지 않고 취소로 판정하며, 취소 상태는 커밋된다

### 12.3 발견한 결함과 수정

**본문이 비어 있거나 끊긴 요청에 서버가 500을 돌려줬다.** 브라우저가 탭을 닫는 순간
전송 중이던 채팅 읽음 표시 요청에서 재현된다(E2E 실행 중 `api.unexpected_error
SyntaxError`가 세 번). 클라이언트가 보낸 잘못된 입력이므로 `400 INVALID_REQUEST_BODY`가
맞다. `handleApiError`에서 본문 파싱 SyntaxError를 400으로 바꾸고 오류 로그도 남기지
않게 했다. 재현 테스트는 `tests/e2e/match-flow.spec.ts`의 마지막 항목이다.

사용자에게 달라지는 것: 통신이 끊긴 요청에서 "요청을 처리하지 못했어요"(500) 대신
"요청 내용을 읽지 못했어요"(400)가 나가고, 서버 오류 로그가 쌓이지 않는다.

**코트 매칭 자체에서는 제품 결함이 나오지 않았다.** 구현된 흐름은 브라우저와 실제 DB
양쪽에서 설계대로 동작했다.

### 12.4 E2E가 실패했던 이유 — 전부 옛 UI 기준이던 테스트

제품이 아니라 테스트가 낡아 있었다. UI를 테스트에 맞춰 되돌리지 않고 테스트를 현재
사용자 조작에 맞췄다.

| 실패한 어서션 | 실제 |
| --- | --- |
| `getByLabel("매칭 날짜").fill(...)` | 날짜·시간은 바텀시트 휠 피커다. 트리거 버튼 → 휠에서 값 선택 → 완료 |
| `getByLabel("은행").fill(...)` | 은행은 목록에서 고르는 `select`다 |
| `getByLabel("매칭 소개글", { exact: true })` | 필수 항목의 접근성 이름에는 `*`가 붙는다(`"매칭 소개글 *"`) |
| `getByLabel(/모집자에게 한마디/)`, 버튼 `"신청하기"` | `모집자에게 보낼 자기소개`, 버튼 `참가 신청` |
| 매칭 상세의 `"모집자가 코트를 예약했어요"` | 상세의 출처 배지는 `6479506`에서 의도적으로 뺐다. 이 문구는 미리보기 시트에 있다 |

휠 피커는 스크롤이 멈춘 뒤에야 선택을 확정해서, 옵션을 직접 클릭하거나 여러 칸을 한
번에 누르면 값이 어긋난다. 한 칸씩 누르고 확정을 기다리며 목표까지 좁히는 헬퍼로 고쳤다.

코트 매칭 채팅 테스트는 간헐적으로 실패했다. 채팅이 서버 응답 전에 임시 말풍선을 먼저
그리기 때문에, 화면에 보이는 것만 확인하고 상대 화면으로 넘어가면 저장 전에 조회하게
된다. 일반 매칭 테스트가 이미 쓰던 대로 POST 응답을 기다리게 했다.

03-3 §6이 걱정하던 WDS `Modal`의 접근성 이름은 문제가 아니었다. `ModalContainer`가
`role="dialog"`이고 `aria-labelledby`가 `ModalNavigation`의 제목 `<h2>`를 가리킨다.

### 12.5 남은 것

- **크론 보류 유지.** 무접속 상태에서 제시간에 취소 알림을 보내는 문제는 해결되지 않았다.
  요청이 들어올 때의 상태 보정만 검증됐다.
- **미확정 정책**(§9): 노쇼·불참 환불, 현장 사고 책임, 문의 처리 절차, 옛 `PARTNER_COURT`
  데이터 정리 방침. 임의로 구현하지 않았다.
- **문서 정합성**: `01-product-overview.md:180`, `02-prd.md:526`, `03-screen-spec.md:47·190`,
  `05-api-spec.md:343`, `06-development-plan.md:805`가 아직 폐기된 모델(일반 모집자가 여는
  제휴 코트 세션)과 폐기된 `Rally On에서 준비한 코트예요` 배지를 기준으로 서술한다.
  사용자 확인 뒤 정리할 항목이다.
- 운영 DB 데이터 변경·배포·요금제 변경은 하지 않았다.
