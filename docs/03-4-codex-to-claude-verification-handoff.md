...
**다음 작업 인계:** 이 문서의 임무는 §12·§13에서 완료됐다. 이어서 할 일과 현재 환경은
...
- ~~**미확정 정책**(§9)~~ — 2026-09-09 사용자 확정. 결정은 `03-2 §3.7`과 §5 표에 있다.
  현장 책임은 운영자, 문의는 기존 1:1 문의로 단일화하되 매칭 참조를 더한다, 옛
  `PARTNER_COURT` 데이터는 보존한다. 환불은 처음에 "노쇼·불참 환불 없음"으로 정했다가
  사용자 요청으로 **단계별 환불**로 바꿨다(§13).
  남은 구현은 (1) 약관의 현장 책임 문구, (2) `SupportInquiry`의 매칭 참조 컬럼과 문의 폼이다.
- ~~**문서 정합성**~~ — 2026-09-09 정리 완료. 단항 문구(01의 코트 유형 표, 02-prd의
  Court Source 표, 03-screen-spec의 배지 표와 §3.5, 05-api-spec의 응답 문구,
  06-development-plan의 인수 기준)를 고치고, 절 전체가 옛 모델로 쓰인 01 §4·§14와
  02-prd §3·§18은 새 모델로 다시 썼다. 코드의 `participationNote`도 함께 고쳤다.
  **남은 곳은 Commerce뿐이다.** 01 §14.2와 02-prd §3.3, `07-court-commerce-design.md`는
  일반 모집자가 코트 이용 총액을 결제하던 전제로 쓰여 있다. 주최자가 운영자로 바뀌면서
  결제 주체와 환불 대상이 달라졌으므로 Commerce를 시작하기 전에 다시 정해야 한다.
  확정된 수수료 정책(운영자 부담 5%, 첫 결제 후 30일 0%)은 사용자 확정값이라 두었다.
- 운영 DB 데이터 변경·배포·요금제 변경은 하지 않았다.

---

## 13. 참가자 취소와 단계별 환불 (2026-09-09, Claude)

§12 검증을 마친 뒤 사용자가 환불 정책을 단계별로 바꾸기로 했다. 정책 정본은
`03-2 §3.7`이고, 여기에는 구현과 검증 범위를 남긴다.

### 13.1 새로 만든 것

앱에는 **참가자가 확정된 참가를 스스로 취소하는 경로가 없었다.** 일반 매칭의
...
- 마이그레이션 `20260909200000_court_match_participant_cancel`
  — `participant_cancelled_at`, `refund_amount_krw`, 알림 종류 `COURT_MATCH_PARTICIPANT_CANCELLED`
- 참가자 화면: 취소 전에 환불 예정 금액과 비율을 보여 주는 2단계 확인
- 운영자 화면: 환불할 참가자에 **보낼 금액**과 `참가자 취소` 표시

### 13.2 정책을 넣으면서 드러난 문제 셋

세 가지 모두 취소 경로가 없던 시절에는 드러날 수 없던 것이다.

1. **판정 이후 취소가 매칭 전체를 취소시켰다.** `cancelForShortfall`이 "지금 확정 상태인
   사람"을 세기 때문에, 시작 2시간 전에 한 명이 취소하면 다음 요청에서 매칭이 통째로
   취소됐다. 상세 화면을 여는 것만으로도 트리거됐다. 판정 인원을 **판정 시점에 확정돼
   있던 사람**으로 바꿨다(`countConfirmedAtJudgement`). 03-2 §3.2의 "시작 3시간 전까지
   확정된 인원으로 판정"이라는 표현과도 더 잘 맞는다. 판정은 1회성 관문이다.
2. **자리를 돌려줘도 쓸 수 없었다.** 정원이 차면 매칭을 `CLOSED`로 닫는데, 취소로 자리가
   비어도 닫힌 채였다. 신청 마감 전이면 다시 모집 중으로 되돌린다(`reopenIfSeatFreed`).
3. **신청 마감 뒤에는 취소도 막혔다.** `withCurrentCourtMatch`가 시작 30분 전 이후의 모든
   요청을 거절해서, 못 가게 된 사람이 자리를 붙잡은 채 "참가 신청과 입금 기한이 지났어요"
   라는 엉뚱한 메시지를 받았다. 취소만 이 관문을 지나가게 했다.
...
| --- | --- |
| 린트·타입 검사 | 통과 |
| Vitest 단위/API/DB 테스트 | **48개 파일 통과.** `court-match-flow.test.ts`는 26개 |
| Playwright 브라우저 E2E | **7개 통과** (참가자 취소 흐름 1개 추가) |

새 테스트가 덮는 것: 한국 시간 날짜 경계(전액/절반/없음과 원 단위 내림), 확정 취소 후
자리 반환과 대화방 제거, 환불 0원 건은 환불 대기가 아니고 계좌도 받지 않음, 입금 확인
전 취소는 철회, 판정 전 취소로 미달이 되면 판정 시점에 전체 취소되고 남은 사람은 전액,
판정 후 취소는 매칭을 취소시키지 않음, 신청 마감 뒤·시작 전 취소 허용, 시작 후 거절,
남의 신청 취소 불가.

### 13.4 배포 전에 확인할 것

**마이그레이션이 로컬 E2E DB에만 적용돼 있다.** 공유 DB에는 배포 시점에 함께 적용해야
한다. `ALTER TYPE ... ADD VALUE`가 들어 있으므로 배포 순서를 확인할 것.
