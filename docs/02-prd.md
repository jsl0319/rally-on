...
> **⚠️ 이 절의 코트 매칭(Court Partner) 서술은 폐기됐다.**
> 운영자가 시간만 공급하고 일반 회원인 세션 모집자가 세션을 열어 참가자를 승인하는
> 모델에서, **운영자가 자기 코트 시간을 예약해 직접 코트 매칭을 열고 참가 신청 승인과
> 계좌이체 입금 확인까지 맡는 모델**로 바뀌었다(2026-09-08).
> **정본은 `03-2-court-match-operator-hosted-redesign.md`다.** 이 절의 역할 분리·상태
> 흐름·화면 문구를 근거로 작업하면 방향이 어긋난다. 일반 매칭(`EXTERNAL_RESERVED`)
> 서술은 그대로 유효하다.
...
긴급 철회는 `SCHEDULE_UNAVAILABLE`, `FACILITY_CLOSED`, `SAFETY_RISK`, `NATURAL_DISASTER`처럼 미리 정한 사유 코드로만 요청한다. 원문 사유·담당자 연락처는 참가자에게 노출하지 않는다. 첫 Pilot의 안내는 Match 상세와 활동 화면의 인앱 고지이며, 푸시·문자·카카오 알림은 동의·개인정보·발송 계약을 확정하기 전에는 즉시 전달 수단으로 약속하지 않는다. 수락자는 서비스 내 Match 방에서 추가 안내를 확인하며, 운영자 연락처는 어느 경우에도 공개하지 않는다.
...
- 전역 하단 메뉴는 `매칭`, `코트 매칭`, `채팅`, `마이` 네 항목이다. `활동`은 마이의 `내 활동 보기`에서만 진입하며, 활동 최상단의 뒤로가기는 마이로 돌아간다. `만들기` 메뉴는 제거하고, 매칭 탭의 `+ 매칭 만들기` CTA와 코트 매칭 탐색의 세션 열기 흐름을 등록 진입점으로 유지한다.
