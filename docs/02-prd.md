...
| 항목 | 내용 |
| --- | --- |
| 문서명 | Rally On MVP Product Requirements Document |
| 문서 상태 | Draft v0.1 |
...
이 문서는 Rally On Core MVP의 기능 범위, 비즈니스 규칙과 완료 조건을 정의한다. 화면 배치와 세부 인터랙션은 화면 명세에서, 데이터 구조는 ERD에서, HTTP 계약은 API 명세에서 구체화한다.
...
- 코트 운영자 계정 및 권한 확인
- 코트 정보와 모집 조건(참가비·정원·승인 방식·입금 계좌) 등록
- 운영자가 자기 코트 시간을 코트 매칭으로 공개
- 일반 사용자의 참가 신청
- 운영자의 승인·입금 확인·참가 확정과 자동 취소·환불 기록

운영자가 코트 매칭의 주최자다. 자기 코트 시간을 예약해 매칭을 열고, 참가 신청을 받고, 참가비 입금을 통장에서 확인해 참가를 확정한다. 일반 사용자는 운영자에게 코트 예약 요청을 보내지 않고 공개된 코트 매칭에 참가 신청한다. 참가 흐름과 상태 정책은 `03-2-court-match-operator-hosted-redesign.md`가 정본이며, 운영자 등록·심사 화면은 `03-1-court-partner-screen-spec.md`를 따른다(그 문서의 세션 모델 서술은 폐기됐다).

공개한 Slot은 모집 중인지·마감됐는지·이용이 끝났는지·취소됐는지를 사람이 이해할 수 있는 상태로 계속 보여 준다. 이 공개 상태는 코트 예약 가능을 뜻하지 않는다. `AVAILABLE`이 공개·모집 중이며 참가 신청을 받는 상태이고, 그 외 상태는 읽기 전용이다. `ALLOCATED`는 옛 모델의 잔재라 새로 쓰지 않는다. 각 Slot에는 운영자를 제외한 참가자 정원과 최소 인원을 운영자가 지정하고, 신청·승인 때 서버가 검증한다.

운영자는 초대 없이 직접 등록을 시작할 수 있다. 자동 확인은 운영자를 즉시 배제하는 수단이 아니라, 공개 전에 사람이 보아야 할 사례를 가려내는 수단이다. 사업자 확인이 끝난 운영자는 코트와 시간대 **초안**을 저장할 수 있지만, 아래 공개 조건을 모두 충족해 `공개 승인`되기 전에는 코트 매칭으로 공개할 수 없다.
...
| 내부 상태 | 운영자에게 보이는 상태 | 가능한 행동 | 코트 매칭 공개 |
| --- | --- | --- | --- |
...
| `PUBLISH_APPROVED` | 운영자 등록이 완료됐어요 | 코트 매칭 공개·참가자 관리 | 가능 |
| `CHANGES_REQUESTED` | 정보를 보완해 주세요 | 수정 후 재확인 요청 | 불가 |
| `REJECTED` | 현재 정보로는 등록이 어려워요 | 사유 확인·정정 후 새 신청 | 불가 |
| `SUSPENDED` | 공개가 일시 중지됐어요 | 운영 문의·보완 | 새 공개 불가 |

`PUBLISH_APPROVED`는 자동 규칙을 모두 충족한 경우 또는 권한 있는 운영 검토자가 승인한 경우에만 전환한다. 검토자는 자신의 신청을 검토할 수 없고, 판정·사유 코드·판정 시각을 기록한다. `INTERNAL_REVIEWER`만 승인된 신청을 `SUSPENDED`로 전환할 수 있으며, 사업자·장소·운영 권한 재확인, 안전 검토, 시설 폐쇄 중 선택형 안전 사유만 남긴다. 중지 즉시 새 Slot 공개·운영자 사진 관리·공개 Slot과 사진 노출을 막는다. 이미 공개된 코트 매칭은 운영자 검증 상태가 바뀌었다고 자동 취소하지 않는다. 다만 실제 공급 불가를 확정한 긴급 철회는 아래 시간 공급 보호 정책에 따라 Match 취소·인앱 안내·사후 검토를 하나의 작업으로 처리한다.
...
| `AVAILABLE` 공개 시간의 오류 | 직접 수정하지 않고 `BLOCKED`로 공개 중지한 뒤 새 초안을 등록 | 기존 시간은 중지 상태와 갱신 시각을 계속 표시. 신청자가 있으면 아래 긴급 철회로 처리 |
| 신청자가 있는 공개 시간의 일반 오류·문의 | 시간·비용·코트 수정과 직접 중지 불가. 운영 검토 요청만 가능 | 코트 매칭은 그대로 유지 |
| 최소 인원 미달 자동 취소 | 시작 3시간 전 판정에서 입금 확인 인원이 최소 인원에 못 미치면 서버가 취소 | 매칭과 신청은 취소로 남고, 입금한 참가자는 환불 대상이 됨 |
| 실제 공급 불가·시설 폐쇄·안전 위험·재난 | 사유 코드를 선택해 긴급 공급 철회. Slot과 연결 Match를 한 트랜잭션으로 취소하고 사후 검토 | 신청·입금 확인 참가자에게 안전한 인앱 안내를 남기고, 입금한 참가자는 전액 환불 대상 |
...
긴급 철회는 `SCHEDULE_UNAVAILABLE`, `FACILITY_CLOSED`, `SAFETY_RISK`, `NATURAL_DISASTER`처럼 미리 정한 사유 코드로만 요청한다. 원문 사유·담당자 연락처는 참가자에게 노출하지 않는다. 첫 Pilot의 안내는 Match 상세와 활동 화면의 인앱 고지이며, 푸시·문자·카카오 알림은 동의·개인정보·발송 계약을 확정하기 전에는 즉시 전달 수단으로 약속하지 않는다. 참가가 확정된 사람은 서비스 내 Match 방에서 추가 안내를 확인하며, 운영자 연락처는 어느 경우에도 공개하지 않는다.

운영자 귀책 긴급 철회가 최근 30일에 2회이거나 시작 24시간 이내에 1회 발생하면 시스템은 새 Slot 공개를 자동 일시 중지한다. 이미 공개된 다른 코트 매칭을 자동 취소하지 않으며, 해제는 플랫폼 검토 후에만 가능하다. 이용자에게는 공개 중지 사실과 안전한 상태만 보이고, 공개적인 신뢰 점수나 원문 사유는 노출하지 않는다.
...
- 활성 운영자는 최소 연 1회, 사업자·주소·운영 주체 변경 신고, 신고·분쟁, 외부 확인 실패가 발생했을 때 재확인한다. 재확인 동안 신규 Slot 공개만 중지할 수 있으며, 이미 공개된 코트 매칭의 처리와 이용자 안내는 별도 운영 절차를 따른다.
...
> **주의:** 이 절은 일반 모집자가 코트 이용 총액을 결제하던 옛 모델을 전제로 쓰였다. 코트 매칭의 주최자가 운영자로 바뀌면서 결제 주체와 환불 대상이 달라졌으므로, Commerce를 시작하기 전에 다시 정해야 한다. 아래 수수료 정책은 사용자가 확정한 값이라 그대로 둔다.

Court Partner Pilot과 분리한 다음 단계다. 일반 사용자 CourtBooking, 운영자의 참가 승인, Rally On의 수기 수납을 만들지 않는다. 상세 설계 기준은 `07-court-commerce-design.md`다.

현재 확정된 제품 정책은 다음과 같다.

- 운영자가 PG의 판매자/하위가맹점 온보딩을 완료하고, PG의 마켓플레이스·지급 기능으로 **모집자 한 명의 코트 이용 총액 결제·환불·정산**을 처리하는 구조를 우선한다. 실제 계약상 책임은 PG 계약과 법무·세무 검토로 확정한다.
- 플랫폼 수수료는 운영자 부담 5%이며, PG 실수수료도 운영자가 부담한다.
- 운영자별 첫 성공 유료 세션 모집자 결제 **승인 시각**부터 30일간 플랫폼 수수료는 0%다. 가입일·세션 시작일이 기준이 아니며, PG 수수료는 무료 기간에도 면제하지 않는다.
- 운영자는 코트 면·이용 시간·최대 인원·사용자 공개 코트 이용 총액을 요금표에 따라 설정한다. 참가자 1인 가격을 직접 입력하지 않으며, 참가자에게는 목표 인원을 기준으로 한 예상 1인 부담만 참고값으로 보여 준다.
- 모집자가 코트 이용 총액을 실제 결제한 뒤에만 유료 세션을 확정한다. 참가 신청 수락·거절과 Match Chat 입장은 결제와 무관하게 기존 `MatchApplication` 규칙을 따른다.
- 참가자는 앱에서 코트 비용을 결제하지 않는다. 현장 비용 분담은 참여자 자율이며, Rally On은 참가자 간 수납·미납·환불·송금을 기록하거나 보장하지 않는다.
- 모집자는 시작 24시간 전까지 코트 이용 총액을 전액 취소할 수 있다. 최소 인원 미달 시에는 같은 마감 전까지 현재 인원으로 진행하거나 전액 취소를 선택하고, 무응답이면 자동 취소한다. 마감 시각 뒤 모집자 사유 자동 환불은 제공하지 않는다.
- 운영자 공급 철회·시설 폐쇄·안전 위험·사용 불가 우천·재난으로 Match를 취소하면 시간과 무관하게 모집자 결제를 전액 환불한다. 참가자는 앱에서 결제하지 않았으므로 참가자 환불은 없다.
- 운영자 요금표 변경은 새 비공개 초안에만 적용한다. 이미 공개·결제된 Slot의 총액·요금표 버전·24시간 취소 마감은 변경하지 않는다.

Commerce 구현은 PG 계약, 법무·세무 검토, 취소·환불 정책, 지원 절차, sandbox 검증이 끝난 뒤에만 시작한다.

### 3.4 서비스 내 Match 채팅

서비스 내 채팅은 모든 Match의 유일한 연락 수단인 Match Chat MVP다. 범용 메신저·개인 DM·일반 파일·음성·접속·입력 중 표시는 제공하지 않는다. 다만 발신 가능한 방 멤버는 JPEG·PNG·WebP 사진을 한 메시지에 최대 3장, 각 5 MiB까지 텍스트와 함께 또는 단독으로 보낼 수 있다. 사진은 비공개 객체로 저장하고 같은 방 멤버의 보호된 읽기 경로만 제공하며, 인물 얼굴·연락처·예약번호·실시간 위치가 보이는 사진은 올리지 않도록 안내한다. 발신자는 최신 일반 메시지에 아직 읽지 않은 다른 현재 방 멤버 수만 숫자로 볼 수 있고, 모두 읽으면 표시하지 않는다. 읽은 시각·누가 읽었는지·온라인 상태는 제공하지 않는다. 상세 정책·보관·신고·운영 방식은 `08-in-app-match-chat-design.md`를 따른다.

- Match별 방 하나만 두며, 모집자와 `ACCEPTED` 참가자만 입장·발신한다. 운영자와 대기·거절·철회·취소 신청자는 입장하지 못한다.
- 첫 수락에서만 방을 서버 트랜잭션으로 만들고, 이후 수락자를 같은 방에 추가한다.
- 시작은 DB 영속 메시지와 화면 활성 중 5초 폴링으로 한다. WebSocket·Redis·외부 실시간 서비스는 별도 승인 전 추가하지 않는다.
- 메시지 신고, `INTERNAL_REVIEWER`의 메시지 숨김·발신 중지·방 읽기 전용 조치, 보관·파기 정책이 출시 전 게이트다.
- 첫 수락에서만 방을 만들고, 해당 Match의 모집자와 수락자만 입장·발신한다. 외부 오픈채팅 링크는 받거나 보여 주지 않는다.
- 전역 하단 메뉴는 `매칭`, `코트 매칭`, `채팅`, `마이` 네 항목이다. `활동`은 마이의 `내 활동 보기`에서만 진입하며, 활동 최상단의 뒤로가기는 마이로 돌아간다. `만들기` 메뉴는 제거하고, 매칭 탭의 `+ 매칭 만들기` CTA와 운영자의 시간 등록 화면을 등록 진입점으로 유지한다.
- 채팅 탭은 내가 만든 Match와 내가 신청해 수락된 Match를 분리해, 실제 방이 있는 항목만 보여 준다. 알림 권한·푸시 알림 설정은 이번 MVP에 포함하지 않는다.
- 채팅 사진은 선택한 뒤 전송할 때만 서버로 올린다. 취소한 로컬 사진은 업로드하지 않으며, 전송 실패로 연결되지 않은 비공개 업로드는 즉시 삭제를 시도하고 실패해도 24시간 뒤 정리한다. 연결된 사진은 메시지 보관·파기 정책을 따르고, 일반 파일 전송·사진 편집·사진별 반응은 제공하지 않는다.
...
3. 일반 매칭은 장소·시간·예상 비용을 신청 전에 명확히 확인한다. 코트를 아직 확보하지 못한 사용자는 운영자가 준비한 시간을 사용하는 코트 매칭으로 이동한다.
...
| Partner Court | 운영자가 Rally On에 등록한 코트. 필수 Court Partner Pilot에서 제공 |
...
8. 직접 예약 코트는 모집자 입력 정보이며 서비스 확인 예약으로 표시하지 않는다. 새 일반 매칭은 코트명·주소·전체 비용 없이 만들 수 없으며, 코트를 확보하지 못한 사용자는 코트 매칭으로 안내한다. 기존 `COURT_TBD` 기록은 이력과 기존 참여자의 채팅·완료 처리를 위해 보존하되 공개 목록·추천·새 신청에서는 제외한다.
9. 외부 예약 번호나 예약 확인서에 포함된 개인정보는 Core MVP에서 수집하지 않는다.
10. 직접 예약 코트의 비용 정산은 당사자 간 진행하며 Rally On이 결제 완료를 보증하지 않는다.
11. 모든 Match는 모집자 입력 링크 없이 만들고, 첫 수락 때 모집자와 수락된 신청자 전용 서비스 내 방으로 연락한다.
12. 직접 예약 코트의 예상 1인 비용은 `ceil(전체 코트 비용 ÷ (모집 인원 + 모집자 1명))`으로 계산하며 수정할 수 없다. 과거 `COURT_TBD` 기록의 비용은 계속 `null`로 표시한다.
...
- 향후 제휴 예약이 확인된 코트에만 `Rally On에서 예약이 확인된 코트예요`라고 표시한다.
...
| 종료 시간 | 필수 | 같은 날짜의 코트 예약 종료 시간. 시작 시간보다 늦어야 함 |
| 지역 | 필수 | 추천과 목록 필터에 사용 |
| 코트 장소 검색 | 선택 | 카카오맵 장소 검색 결과를 선택하면 코트장 이름과 주소를 자동 입력. 검색 결과가 예약·운영 상태를 보증하지 않으며 직접 입력을 항상 제공 |
...
| 연락 수단 | 서비스 내 Match 채팅 | 외부 오픈채팅 링크를 받거나 저장하지 않는다. 첫 수락 뒤 해당 Match 방에서만 연락한다. |
...
- 수락된 신청자는 자신의 신청 목록에서 성사 상태와 채팅방 진입 상태를 확인한다.
...
| PARTNER_COURT | 운영자가 자기 코트 시간을 예약해 직접 여는 코트 매칭 | 코트 매칭 탭 전용. 상세에 출처 배지를 두지 않는다 | 필수 Court Partner Pilot |
| COURT_TBD | 과거 코트 미정 기록 | 코트와 비용을 함께 정해요 | 신규 생성 불가, 이력 전용 |
...
- 전화번호와 이메일은 공개하지 않는다. Match 방 상태와 안전한 진입 경로만 모집자와 ACCEPTED 신청자에게 반환한다. 메시지·신고 원문은 로그·분석 이벤트에 넣지 않는다.
...
- 제휴 코트의 대표 사진은 `PUBLISH_APPROVED` 운영자가 직접 제공하거나 사용할 권한이 있는 시설 사진만 최대 3장 관리한다. JPEG·PNG·WebP만 10 MiB 이하로 받고 서버가 파일 시그니처를 확인한다. 대표 1장만 로그인·온보딩을 마친 이용자에게 비공개 객체의 보호된 읽기 경로로 보이며, `운영자 제공 사진` 출처를 함께 표시한다. 사진은 예약 가능, 운영자 심사 완료, Rally On의 시설 보증을 뜻하지 않는다.
...
3. 모집자가 외부 예약 코트 정보와 비용을 모두 포함해 매칭을 등록할 수 있다. 코트를 확보하지 못한 사용자는 코트 매칭의 운영자 공급 시간에서 세션을 연다.
...
11. 모집자와 ACCEPTED 신청자만 서비스 내 Match 방에 입장할 수 있다.
...
| CORE-03 | 매칭 성사 후 연락 | 모든 Match에서 모집자와 ACCEPTED 신청자만 같은 Match의 서비스 내 채팅방 사용. 텍스트와 방 멤버 전용 사진만 허용 |
| CORE-04 | 성별 및 연령 | 수집하지 않음 |
| CORE-05 | 일반 매칭의 코트 정보 | 새 일반 매칭은 외부 예약 코트명·주소·전체 비용이 필수. `COURT_TBD`는 신규 생성·공개 목록·추천·새 신청에서 제외하고, 과거 기록의 이력·기존 참여자 채팅·완료 처리만 유지 |
...
- 코트 운영자: 코트·시간 등록과 공개 관리, 코트 매칭 주최, 참가 승인과 입금 확인, 환불 표시
- 참가자: 공개된 코트 매칭의 조건을 확인하고 참가 신청, 참가비 이체, 취소 시 환불 계좌 입력

일반 사용자가 운영자에게 코트 예약 요청을 보내지 않는다. 코트를 준비한 사람과 참가를 확정하는 사람이 같아야 코트는 비었는데 매칭만 열려 있거나 사람은 모였는데 코트를 못 쓰는 어긋남이 생기지 않는다. Rally On은 참가비를 받거나 옮기지 않으며, 입금·환불 여부를 확인하지도 않는다.
...
`visibility = PUBLIC`은 `status`와 별개다. Pilot에서 새 Slot은 비공개 초안으로 시작하고, 한 번 공개한 Slot은 `ENDED`, `BLOCKED`, `CANCELLED`가 되어도 상태와 갱신 시각을 계속 표시한다. 공개 Slot의 사용자 표시 필드는 직접 수정하거나 다시 `AVAILABLE`로 열 수 없으며, 오류는 공개 중지 후 새 초안으로 정정한다. `AVAILABLE`이 공개·모집 중이고, `ALLOCATED`는 옛 모델의 잔재라 새로 쓰지 않는다. 최소 인원 미달 자동 취소와 운영자의 긴급 공급 철회는 Slot과 연결 Match를 함께 취소한다.

참가 신청 상태는 `MatchApplication`에 유지하되 코트 매칭은 단계가 하나 더 있다. 승인은 확정이 아니라 입금 대기(`ACCEPTED`)이고, 운영자가 통장에서 입금을 확인해야 참가가 확정(`CONFIRMED`)된다. 운영자는 승인·입금 확인에 필요한 최소 정보(닉네임·성별·신청 시각·입금 상태)를 보며, 실력 프로필과 신청 메시지는 `운영자 승인` 방식에서만 본다.

카드 결제·PG 정산은 이 Pilot 범위에 포함하지 않는다. 참가비는 참가자가 운영자 계좌로 직접 이체하고 앱은 진행 상황만 기록한다. Commerce를 구현하더라도 일반 사용자 코트 예약 모델을 다시 도입하지 않으며, `07-court-commerce-design.md`의 모집자 단일 결제 설계는 주최자가 바뀐 지금 모델에 맞춰 다시 정해야 한다.

운영자 등록은 자율 신청으로 제공하되, `자동 확인 중`, `코트 초안을 만들 수 있어요`, `추가 확인이 필요해요`, `운영자 등록이 완료됐어요`, `현재 정보로는 등록이 어려워요`를 사용자 언어로 분리해 보여준다. 사업자 확인 후에는 비공개 초안을 저장할 수 있어도, `PUBLISH_APPROVED` 전에는 코트 매칭 공개를 허용하지 않는다. 운영 검토의 기준, 권한, 이력과 재확인 정책은 3.2의 검증 정책을 따른다.
...
사용자 승인: 매칭 제목 입력을 없애고 목록·상세 대표 이름은 테니스장명을 사용한다. 일반 개설에 게임 유형(혼복·남복·여복·기타)을 추가한다. 이후 남녀별 정원 기능이 도입되면서 성별은 테니스 프로필의 필수 입력으로 확정됐다(이 문단의 초기 결정인 총원 방식·성별 미수집은 더 이상 적용되지 않는다). 게임 유형은 기존 Player Skill과 Match Purpose를 변경하지 않는다. 정산 계좌(은행·계좌번호·예금주)는 선택 입력, 모집자와 ACCEPTED 참가자에게만 공개한다. 앱 밖 비용 분담 안내이며 결제·송금·입금 확인은 제공하지 않는다. 매칭 소개글은 넓은 직접 입력 영역으로 제공하고 자동 생성 기능을 제거한다. 상세 흐름은 화면 명세 12절을 따른다.
