...
환불 계좌 수정과 환불 완료 표시는 같은 Match 행 잠금 안에서 처리한다. 잠금을
얻은 뒤 완료 여부를 다시 읽으며, 이미 완료된 건의 계좌 변경은 거절한다.
두 요청이 겹쳐도 완료 이후 계좌가 덮어써지지 않아야 한다.
...
- **취소는 시작 전까지 언제든 받는다.** 환불이 없는 구간에서도 마찬가지다. 못 가게 된
  ...
  신청 마감(시작 30분 전) 이후에도 취소만은 받는다.
- **자리는 언제 취소하든 돌려준다.** 정원이 차서 마감된 매칭이라면 신청 마감 전에
  한해 다시 모집 중으로 되돌린다. 마감된 채로 두면 돌려준 자리를 아무도 쓸 수 없다.
- 입금이 확인되지 않은 신청(`PENDING`·`ACCEPTED`)의 취소는 그냥 철회다. 돌려줄 것이
  없다. 이미 이체했는데 확인 전이라면 통장 대조가 필요하므로 화면에서 문의로 안내한다.
...
참가자의 연락처를 노출하지 않는다는 경계는 그대로 유지한다.
