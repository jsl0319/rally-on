...
**결정: 최초 진행 판정은 자동 처리한다.** 운영자 판단을 기다리지 않는다. 2026-09-12 승인한 R05의 새 공개 경기에는 총원과 실제 성별 구성을 함께 적용한다. 최초 통과 후 구성 부족은 자동 취소하지 않고 운영자 조치로 처리한다. 상세와 기존 공개 경기 보존 범위는 [03-11](03-11-court-match-composition.md)을 따른다.
...
2026-09-12 확정: 새 등록·첫 공개에서 복식은 최소 4명 이상, 기타는 최소 2명 이상으로 제한한다. `남 정원 + 여 정원 = 최대 인원`을 강제하고, 혼복은 남녀 정원 각각 2명 이상, 남복·여복은 해당 성별 자리만 허용한다. 운영자를 선수에 자동 포함하지 않는다.

진행 판정은 설정한 최소 인원도 만족해야 한다. 혼복은 확정 남 2명·여 2명 이상, 남복·여복은 해당 성별 확정 인원이 설정 최소 인원 이상이어야 한다. `ACCEPTED`와 입금 알림만 보낸 신청은 확정으로 세지 않는다. 이전 공개 경기는 버전 0의 기존 총원 판정을 유지한다. 새 공개는 버전 1과 최초 통과 기록을 보존한다([03-11](03-11-court-match-composition.md)).
...
**환불받을 계좌는 실제 반환 잔액이 생긴 뒤에만 받는다.** 취소·종료뿐 아니라 활성 신청의 초과 입금도 포함한다. 계좌이체는 앱 밖에서 일어나
참가자가 어느 계좌에서 보냈는지 서버가 알 수 없다. 입금 알릴 때 미리 받아 두면
대부분 쓰지 않을 계좌 정보를 전원에게서 모으게 되므로, 필요해진 사람에게만 받는다.

### 3.10 탈퇴·업무 종료 후 거래 처리 — 확정 (2026-09-11)

탈퇴는 미처리 환불을 이유로 차단하지 않는다. 시작 전 코트 신청의 취소와 예상 반환액을 미리 보여 주고 재확인한 뒤 탈퇴와 같은 작업으로 처리한다. 확정 참가 취소의 기존 날짜별 기준은 유지하며, 미확정 종료와 초과 입금은 §3.9를 따른다. 탈퇴 후 같은 카카오 인증으로 본인의 기존 코트 거래·환불 계좌·문의에만 제한 접근한다. 일반 프로필·매칭 신청·채팅 권한은 재개하지 않는다.

비활성 운영자의 신규 모집·승인·입금 확정은 중단한다. 원래 운영자 ID는 보존하고 내부 담당자가 근거와 함께 잔여 거래를 인계받는다. 기존 송금의 고정 계좌·금액과 결과 이력은 그대로 이어 간다. 계정 또는 담당 권한이 종료되면 다른 내부 담당자가 문의와 거래를 재배정받을 수 있다. 시작 전 제공 불가가 확인된 경기의 현재 참가자는 운영 사유 취소·실제 수령액 전액 반환으로 처리한다. 과거 자발 취소 기록은 덮어쓰지 않는다.

화면, 잠금 순서, 권한과 미확정 보관 정책은 [03-10](03-10-account-transaction-continuity.md)을 따른다. 보관 기간·재가입 및 R06/R10 제공 불가 사유 우선순위 전체를 임의 확정하지 않는다.
...
- 앱이 **전액**을 환불하는 경우는 앱 사유 취소, 즉 **최소 인원 미달 자동 취소**와
  **운영자 긴급 공급 철회**, **경기 구성 부족으로 확인된 제공 불가 취소**다(§3.9, [03-11](03-11-court-match-composition.md)). 이때 `refundAmountKrw`는 비어 있고 전액을 뜻한다.
- 노쇼(취소하지 않고 나타나지 않음)는 환불하지 않는다. 예외는 1:1 문의로 운영자가
  개별 판단한다.

**판정 통과 후 참가 취소로 전체 자동 취소를 반복하지 않는다.** 최초 판정은 시작 3시간 전의 관문이며 통과 기록과 당시 인원을 보존한다. 새 정책 경기에서 현재 구성이 부족해지면 운영자 홈·시간 관리·상세 및 참가자 상세에 **운영자 조치 필요**를 표시한다. 기존 신청 마감·입금·확인 기한 안에서 보충하고, 구성이 충족되면 표시를 해제한다.

실제 경기 제공이 불가능하다고 확인한 운영자는 시작 전에 근거를 남겨 경기 구성 부족으로 취소할 수 있다. 남은 신청은 취소하고 실제 수령액 전액을 반환 대상으로 남긴다. 먼저 자발적으로 취소한 신청의 날짜별 정책·처리 중 송금은 그대로 유지한다. 구성 문제를 시설 문제로 잘못 접수하거나 남은 참가자의 당일 자발적 취소로 유도하지 않는다. 시작 후 중단·부분 제공 및 사유 우선순위 재심은 이번 범위에 포함하지 않는다.
...
| 진행 구성 미달 | 시작 3시간 전, `CONFIRMED` 총원·유형별 성별 구성 판정. 기존 공개는 버전 0 유지(§3.2·§3.6) |
...
| 판정 후 취소 | 최초 통과 유지. 새 정책의 현재 구성이 부족하면 조치 필요 표시 → 기한 내 보충 또는 운영자 제공 불가 취소·남은 참가자 전액 반환(§3.7·03-11) |
