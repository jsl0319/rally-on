...
- 시작 N시간 전(기본 12시간)에 미달이면 운영자에게 `진행 / 취소` 결정을 요청
- 운영자가 취소하면 이미 입금한 참가자에게 **환불 안내** 상태가 뜬다
- 환불 자체는 앱 밖 계좌이체다. 앱은 `환불 대기 → 운영자가 환불 완료 표시`까지만
  기록하고, 실제 송금을 보증하지 않는다는 문구를 함께 둔다

환불 흐름이 부담스러우면 **파일럿은 최소 인원을 "안내값"으로만 두고 자동 취소를
아예 만들지 않는 선택지**도 있다. 다만 그 경우 최소 인원을 화면에 약속처럼 쓰면
안 된다.

### 3.3 운영자 승인을 **옵션으로** 두는 걸 권한다
...
운영자 계좌를 Slot마다 두는 대신 **`Court`에 한 번만 저장하고 Slot이 참조**하는 것도
가능하다. 한 시설의 계좌가 매번 같다면 그쪽이 입력 부담이 적다. 다만 계좌를 바꿔도
과거 기록은 그대로 남아야 하므로, Slot 공개 시점에 **값을 복사해 스냅샷으로** 남기는
방식을 권한다.
...
1. 코트 매칭 상세 경로를 `/matches/[id]`로 둘지, 코트매칭 탭 안(`/partner-sessions/[id]`)으로
   옮길지. 매칭 탭과 완전히 분리하려면 후자가 맞지만 채팅·신청 화면의 링크가 함께 움직인다
2. 운영자 승인을 옵션으로 둘지, 항상 운영자 승인으로 갈지(§3.3)
3. 최소 인원 미달 시 자동 취소를 만들지, 안내값으로만 둘지(§3.2)
4. 환불 상태를 앱에서 추적할지
5. 계좌를 `Court`에 한 번 저장할지 Slot마다 받을지(§4.2)
6. 이미 만들어진 `PARTNER_COURT` 매칭 데이터를 어떻게 할지(파일럿이면 정리해도 무방)
7. 운영자에게 보여 줄 참가자 정보 범위(§3.8)
