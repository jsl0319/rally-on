...
게임 설정은 혼복·남복·여복·단식·랠리·기타 중 하나를 선택한다. 혼복·남복·여복의 뜻을 설명한다. 일반 개설 화면에서는 선택을 요구하며 이전 클라이언트·기존 기록은 미지정 상태를 유지한다. 기존 플레이 목적·실력 프로필은 별도로 유지한다. 게임 유형은 희망 형식 안내이며 성별 수집·남녀별 정원·성별 자동 신청 제한을 도입하지 않는다.
