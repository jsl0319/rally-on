...
코트 매칭은 운영자가 자기 코트 시간을 예약해 직접 여는 Match이며 코트 매칭 탭에서만 보여 준다. 탭 자체가 운영자 공개 목록이라 배지가 새 정보를 주지 않아 상세의 출처 배지는 뺐다(근거는 커밋 `6479506`, 03-1 §10.2·§15). 직접 예약 코트와 같은 아이콘, 색상 또는 `인증` 문구를 사용하지 않는다.
...
| `PARTNER_COURT` | 배지 없음 | 코트 매칭 탭 전용. 참가 신청은 코트 매칭을 연 운영자에게 보내요. | 상세에 출처 배지를 두지 않는다(`6479506`) |
...
3. 선택된 목록의 제목·한 줄 안내
4. 외부 코트 견본 사진 또는 제휴 코트 운영자 사진이 있는 MatchCard
...
- 외부 코트는 견본 사진 5종 중 코트명·주소 기준으로 고정 선택하며 `견본 이미지`로 표시한다. 제휴 코트는 운영자 사진을 사용하고, 사진이 없으면 기본 코트 일러스트를 표시한다.
- `모집 중` StatusChip
- `🌱 초보자 환영`
- 테니스장명
- 날짜, 시작 시간, 종료 시간
...
- CourtSourceBadge
- 대표 사진과 출처: 외부 코트는 `견본 이미지`, 제휴 코트는 `운영자 제공 사진`. 견본은 실제 해당 코트의 사진을 뜻하지 않는다.
- 코트장 이름
- 주소
- 코트 번호—입력된 경우만
- 전체 코트 비용
...
위치 정보에 `카카오맵 길찾기` 버튼을 제공한다. 서버에서 저장된 코트 주소를 카카오 로컬 API로 좌표 변환하고, 카카오맵 공식 길찾기 URL로 이동한다. 지도 미리보기는 추가하지 않는다.
...
1. 제목 `참가 신청`과 닫기 버튼
2. 코트명, 아이콘을 곁들인 일시·주소 요약
3. 참가비와 `별도 정산` 안내를 한 줄로 표시
4. `모집자에게 보낼 자기소개` 선택 입력란
5. Primary CTA: `[참가 신청]`, Secondary: `[취소]`

개별 테두리 정보 카드와 내 프로필 요약은 생략한다. 자기소개는 넓은 연회색 입력란으로 제공하며 최대 200자와 현재 글자 수를 표시한다. 신청 중에는 입력·닫기·CTA를 비활성화하여 중복 제출을 막는다. 오류가 발생하면 입력을 유지하고, 중복 신청이면 신청 내역 링크를 제공한다. 메시지가 채팅방에 자동 전송된다는 안내는 제공하지 않는다.
...
코트를 이미 예약한 사용자가 짧고 명확한 단계로 참가자 모집을 등록하게 한다. 새 일반 매칭은 `EXTERNAL_RESERVED`만 만들 수 있으며, 코트를 아직 확보하지 못한 사용자는 홈의 `코트 매칭` 경로로 이동한다. 코트 미정 일반 매칭을 만드는 선택 카드나 입력은 제공하지 않는다.

### 12.3 입력 구조

일정·코트, 게임 설정·모집, 비용·정산 정보·매칭 소개글을 한 페이지에 순서대로 배치한다. 하단 `[미리보기]`는 입력을 검증하고 바텀시트를 연다. 시트의 `[매칭 공개하기]`를 눌러야 API를 호출한다. 등록 오류는 시트 내부에, 입력 오류는 폼에 표시한다.

별도 매칭 제목을 입력하지 않는다. 목록·상세·미리보기의 대표 이름은 테니스장명이며, 과거 코트 미정 기록만 `코트 미정`으로 표시한다. 제휴 코트 세션 개설에서도 제목 입력을 제거한다.

게임 설정은 혼복·남복·여복·기타 중 하나를 선택한다. 혼복·남복·여복의 뜻을 설명한다. 일반 개설 화면에서는 선택을 요구하며 이전 클라이언트·기존 기록은 미지정 상태를 유지한다. 기존 플레이 목적·실력 프로필은 별도로 유지한다. 게임 유형은 희망 형식 안내다. 이후 남녀별 정원이 도입되어 성별은 프로필 필수 입력이 됐고, 남녀별 정원이 설정된 매칭에서는 해당 성별의 자리가 남아 있어야 신청할 수 있다.

정산 정보는 은행·계좌번호·예금주를 선택 입력한다. 입력하면 세 항목이 모두 필요하다. 모집자 미리보기와 모집자·수락된 참가자의 상세에서만 계좌를 표시한다. 목록·대기 신청자·제3자에게는 반환하지 않는다. 앱 밖 송금 안내이며 계좌 검증·송금·입금 확인 기능은 없다.

매칭 소개글은 선택 입력, 300자 제한, 가로 전체 너비와 최소 6줄 입력 영역으로 제공한다. 소개 자동 생성 버튼을 제거하고 브라우저 자동완성도 끈다.
...
- 연령대·NTRP·외부 코트 검색/예약은 제공하지 않는다. 성별은 남녀별 정원 판단에만 사용하고 필터로 제공하지 않는다. 초보자 친화적인 플레이 목적과 상대 선호 선택, 모집자를 포함한 예상 총 참여 인원·예상 1인 비용으로 충분히 판단할 수 있게 한다.
...
닉네임은 마이 화면이 아니라 이 화면에서 수정한다. 프로필이 곧 상대에게 보이는 내 정보이므로 한 곳에서 함께 다룬다. 닉네임이 바뀐 경우에만 저장 시 먼저 반영하고, 이미 사용 중인 닉네임이면 그 자리에서 알리고 나머지 프로필은 저장하지 않는다.

성별은 필수 입력이다. 남녀별 정원이 있는 매칭의 신청 가능 여부를 판단하는 데 쓰이므로 `나중에 입력` 같은 미입력 상태를 두지 않는다. 저장 API도 성별 없는 요청을 거부한다. 다만 이 정책 이전에 만들어진 프로필에는 성별이 없을 수 있어 `TennisProfile.gender` 열은 nullable로 두고, 해당 사용자가 프로필을 수정할 때 성별을 채우게 한다.
...
받은 신청·수락·거절 이벤트가 발생하면 관련자에게 서비스 내 알림을 남긴다. 외부 push·이메일 발송은 Core MVP 범위가 아니며, 마이 화면 종 아이콘과 `/my/notifications` 목록으로만 제공한다. `계정 메뉴 > 매칭 알림`에서 끄면 이후 이벤트에 대한 알림 생성을 멈춘다(기존 알림은 남는다). 숫자는 검토가 필요한 항목 수만 보여주고 승률·순위 등 비교 정보는 포함하지 않는다.

### 15.8 공지사항 · 1:1 문의

공지사항은 운영팀이 등록한 안내를 최신순으로 보여주는 읽기 전용 목록이다(Core MVP에는 등록 화면이 없어 `db:seed:announcements` 또는 DB 직접 입력으로 운영한다). 1:1 문의는 사용자가 서비스 내에서 문의를 남기고 접수 여부를 확인하는 용도로, 특정 외부 연락채널을 안내하지 않는다. 외부 채널을 통한 운영 문의 절차는 `docs/07-m9-limited-user-test.md`에 별도로 정의된 채널이 확정되기 전까지 이 문서에 적지 않는다.

### 15.9 회원 탈퇴

탈퇴는 계정을 소프트 삭제한다. `User.status`를 `WITHDRAWN`으로 바꾸고 `withdrawnAt`을 기록하면 이후 모든 로그인·API 요청이 즉시 차단된다. 닉네임과 매칭·신청 이력은 삭제하거나 익명화하지 않고 그대로 보존한다(다른 참가자와의 매칭 기록·정산 근거를 남기기 위함). 탈퇴 시점에 모집 중이던 매칭을 자동으로 취소하거나 대기 신청자에게 알리는 처리는 아직 없다—별도 요구사항으로 다룬다. 데이터 완전 삭제·익명화 정책과 보관 기간은 개인정보 처리방침 개정과 함께 별도로 확정한다.
...
시간 선택 시 종료 시간은 시작 시간보다 반드시 늦어야 한다. 같은 시간·역전된 시간은 휠 시트에서 이유를 안내하고 완료 버튼을 비활성화한다. 종료 시간을 먼저 선택한 경우 시작 시간 선택에도 동일한 제약을 적용한다. 미리보기와 서버 생성 검증도 유지한다.

매칭 개설은 상단 `매칭 개설` 제목 아래 기본 정보 카드부터 시작한다. `테니스 메이트 모집 / 필요한 정보를 입력해 주세요` 소개 블록은 표시하지 않는다. 코트 번호(선택, 최대 50자)는 직접 입력 다이얼로그 대신 종료 시간 아래 기본 폼에 배치하여 검색으로 코트를 선택한 경우에도 입력할 수 있다. 미리보기의 코트 요약에도 입력한 번호를 표시한다.

일반 매칭 개설과 미리보기에는 추가 모집 인원만 표시한다. 모집자가 별도로 확보한 참가자가 있을 수 있으므로 `나를 포함해 총 n명`, `총 n명 예정`처럼 실제 전체 참가 인원을 추정하는 문구는 제공하지 않는다. 남녀별 모집 수와 신청·수락 정원 관리는 그대로 유지한다.

시간 선택은 선택한 날짜와 한국 시간(Asia/Seoul)을 기준으로 현재보다 늦은 00·30분만 확정한다. 처음 열 때 가능한 가장 이른 시간을 제안하며, 가능한 시간이 없으면 날짜나 상대 시간 변경을 안내한다. 선택창이 열린 동안 시각을 갱신하고 완료 클릭·미리보기·공개 제출에서 재검증한다. 날짜 변경으로 과거가 된 기존 시간은 비운다. 생성 API로 보낼 때도 +09:00을 명시해 기기의 시간대와 무관하게 동일한 일정으로 저장한다.

### 2026-09-08 목록·상세 시각 정리

- 목록은 동일 너비의 왼쪽 썸네일과 오른쪽 상태 배지·지역·코트명·날짜·시간·게임 유형·남은 자리를 사용한다. 긴 코트명은 두 줄까지 표시하고 주소 전체는 상세에서 확인한다. 비용과 코트 확보 출처는 카드 하단에 구분해 표시한다. 추천 이유가 없는 카드는 빈 공간을 예약하지 않는다.
- 상세는 큰 대표 사진, 상태와 유형 배지, 코트명·주소, 기본 정보, 소개글, 추천 이유, 플레이, 모집자, 위치, 비용 순서다. 날짜·시간·모집 인원·코트 번호는 아이콘과 항목명/값으로 정렬한다.
- 외부 코트의 목록·상세 대표 사진에는 로컬 견본 5종을 사용한다. 기존 업로드 데이터/API는 보존하며 제휴 코트 사진으로 대체 표시하지 않는다. 같은 코트명과 주소는 항상 같은 견본을 사용한다. 견본 이미지 배지와 대체 텍스트로 실제 장소 사진과 구분한다.
- 사진 파일·촬영자·출처·라이선스는 `public/images/court-samples/credits.json`에 기록한다.
- 신청 가능한 상세는 하단에 참가비와 신청 버튼을 함께 표시한다. 기존 신청·권한·마감·채팅 동작, 정산 정보 공개 조건은 유지한다. 외부 코트 참가비는 확정 입력값, 제휴 코트는 예상 1인 비용이다.

### 2026-09-08 상세·목록 후속 개선

- 상세 상단의 `모집자가 코트를 예약했어요` 문구는 제거하고 그 자리에 `매칭 소개글` 제목과 소개 본문을 배치한다. 소개글을 하단에서 중복 표시하지 않는다. 코트 출처 모델과 견본/운영자 사진 표시는 유지한다.
- 기본 정보의 날짜·시간은 `일시` 한 행으로 묶고 좁은 화면에서는 값만 자연스럽게 줄바꿈한다. 모집자 정보는 `구력`, `랠리 수준`, `게임 경험`을 명시한다.
- 플레이 배지 색상: 편하게 공 주고받기 초록, 랠리 파랑, 스트로크 연습 보라, 게임 입문 주황, 게임 분홍. 목록·상세에서 같은 규칙을 공유하며 텍스트도 함께 표시한다.
- 목록은 일시를 한 줄 우선으로 표시하고, 별도 예약 안내 행을 제거한다. 남은 자리·참가비를 한 행에 묶고 추천 이유는 작은 보조 문구로 유지한다.
- 길찾기는 버튼 클릭 시에만 좌표를 조회한다. 기존 서버 전용 카카오 REST 키를 사용하며 DB 변경·지도 SDK·새 의존성은 없다. 명확한 주소 결과 1건만 연결하고 모호한 주소/행정구역/조회 실패 시 오류, 재시도 및 카카오맵 위치 검색 링크를 제공한다. `COURT_TBD`에는 버튼을 표시하지 않는다.

### 목록 배지의 역할과 추천 안내

- 상단에는 모집 상태·초보자 환영만 배지로 표시한다. 내가 개설한 매칭은 오른쪽에 연필 아이콘과 `내가 개설` 보조 문구를 두어 상태 배지와 구분한다.
- 게임 유형(기타·혼복·남복·여복)은 플레이 배지(랠리·게임 입문 등)와 같은 행의 첫 번째에 중립 색상으로 표시한다. 플레이별 색상은 유지한다.
- 카드 하단 비용 옆에는 사람 아이콘과 굵은 파란 `남은 자리 N명` 배지를 표시한다. 남녀별 정원은 남·여의 남은 인원을 명시한다. 모집 중이 아닌 매칭에는 잔여 정원 대신 실제 상태를 표시한다.
- `랠리 수준이 비슷해요`는 조회자·모집자의 현재 자기입력 프로필 랠리 단계가 같거나 한 단계 차이라는 뜻이다. 경기 측정이나 영상 분석 결과를 의미하지 않는다. 동일 단계와 인접 단계의 내부 추천 점수는 구분하지만 사용자 문구는 `랠리 수준이 비슷해요.`로 통일한다. 내가 개설한 매칭에는 자기 자신과의 추천을 계산하거나 표시하지 않는다.
