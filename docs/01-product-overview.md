...
Rally On은 사용자가 상대방의 실력과 기대하는 플레이를 쉽게 이해하도록 돕는다. 또한 코트를 이미 예약한 사용자가 참가자를 모집할 수 있도록 하고, 코트 운영자가 자기 코트 시간을 예약해 직접 코트 매칭을 열 수 있게 하여 초보자가 장소와 상대를 각각 찾아야 하는 부담을 줄인다.
...
코트 매칭에서는 코트 운영자를 두 번째 사용자 집단으로 추가한다. 운영자는 시간만 공급하는 공급자가 아니라 **코트 매칭의 주최자**다. 자기 코트 시간을 예약해 매칭을 열고, 참가 신청을 받고, 참가비 입금을 통장에서 확인해 참가를 확정한다.

대표적인 특성은 다음과 같다.

- 비어 있는 코트 시간을 초보자 매칭으로 직접 채우고 싶음
- 전화, 메시지와 수기 장부로 코트 시간 운영을 관리하고 있음
- 사업자와 실제 테니스장 정보를 안전하게 제출해 공급 주체를 확인받고 싶음
- 게스트 한 명이 내는 참가비를 정하고 계좌이체로 직접 받고 싶음
- 신청을 선착순으로 받을지 직접 보고 정할지 매칭마다 고르고 싶음
- 누가 보낸 돈인지 통장 내역과 대조할 수 있기를 원함
- 공개한 시간의 모집·입금·확정·취소 상태를 한곳에서 확인하고 싶음
- 카드 결제와 정산 연동은 운영이 검증된 이후 단계적으로 도입하기를 원함

운영자는 테니스 실력을 평가하는 역할이 아니다. 승인은 성별·정원·기한 같은 모집 조건을 확인하는 일이며, 실력 프로필과 신청 메시지는 `운영자 승인` 방식을 고른 매칭에서만 본다. Rally On은 참가비를 받거나 옮기지 않는다. 입금과 환불은 운영자와 참가자 사이의 계좌이체이고 앱은 그 진행 상황을 기록할 뿐이다.
...
반대로 외부에서 코트를 먼저 예약한 사용자는 정해진 시간까지 참가자를 모집해야 한다. Rally On은 **이미 예약된 코트와 참가자를 연결하는 흐름**과 코트 운영자가 자기 코트 시간으로 직접 매칭을 여는 흐름을 함께 제공한다. 일반 사용자가 운영자에게 코트 예약 요청을 보내지는 않는다. 참가비는 참가자가 운영자 계좌로 직접 이체하며 앱 내 결제와 정산은 별도 단계로 유지한다.
...
| 코트 매칭 | 운영자가 자기 코트 시간을 예약해 직접 여는 매칭 | 코트 매칭 탭에서만 보여 준다(배지 없음) | 필수 Court Partner Pilot |

직접 예약 코트는 모집자가 제공한 정보를 기반으로 하며 Rally On이 예약 사실을 검증한 것으로 표현하지 않는다. 코트 매칭은 공개 승인된 운영자가 자기 코트 시간으로 연 Match만 표시한다.

일반 `매칭`에서는 모집자가 코트의 이름, 위치, 예약 날짜·시작·종료 시간과 게스트 참가비를 모두 입력한 뒤에만 공개한다. 코트 이름과 주소는 카카오맵 장소 검색으로 채울 수 있지만, 검색 결과가 예약·운영 상태를 보증하지 않으며 직접 입력도 항상 가능하다. 코트를 아직 확보하지 못한 사용자는 코트 정보가 비어 있는 모집 글을 만들지 않고 `코트 매칭`에서 운영자가 준비한 시간을 확인한다. 게스트 참가비는 게스트 한 명이 내는 고정 금액이다. 코트 총액을 인원으로 나누지 않으며, 코트 매칭에서는 운영자가 이 금액을 정한다. 참가자 간 비용 정산은 앱 밖에서 진행한다. 기존 `COURT_TBD` 기록은 이 정책 전환 뒤에도 이력·기존 참여자 채팅·완료 처리를 위해 보존한다.
...
모집자가 직접 예약한 매칭과 운영자가 여는 코트 매칭을 구분한다. 두 흐름은 탭을 나누어 보여 주며, 일반 사용자는 운영자에게 코트 예약 요청을 보내지 않고 공개된 코트 매칭에 참가 신청한다. 참가 확정과 비용 수령 주체가 다르므로 사용자가 이를 오해하지 않도록 문구를 다르게 쓴다.
...
이 목록 중 코트 운영자 등록과 코트 매칭은 Core MVP 다음의 필수 Court Partner Pilot에서 구현한다. 나머지 기능과 결제·환불·정산은 실제 사용자 행동·운영 가능성 및 별도 승인 후 검토한다.
...
```mermaid
flowchart TD
    A["운영자가 자기 코트 시간을 예약해 코트 매칭 공개"] --> B["초보자 참가 신청"]
    B --> C["선착순 자동 승인 또는 운영자 승인"]
    C --> D["참가자가 운영자 계좌로 참가비 이체"]
    D --> E["운영자가 통장에서 확인하고 참가 확정"]
    E --> F["함께 플레이"]
```

코트를 준비한 사람과 참가를 확정하는 사람이 같다. 그래서 코트는 비었는데 매칭만 열려 있거나, 사람은 모였는데 코트를 쓸 수 없는 어긋남이 생기지 않는다. 최소 인원을 채우지 못하면 시작 3시간 전에 자동으로 취소하고, 입금한 참가자는 환불 대상으로 남긴다. Rally On은 참가비를 받거나 옮기지 않으며 입금·환불 여부를 확인하지도 않는다. 결제·정산 연동은 운영이 검증된 뒤 별도로 검토한다. 상세 정책은 `03-2-court-match-operator-hosted-redesign.md`가 정본이다.

운영자 등록·심사 화면은 `03-1-court-partner-screen-spec.md`에서 정의한다. 다만 그 문서의 세션 모델 서술은 폐기됐으므로, 참가 흐름은 `03-2`를 따른다.

운영자는 초대 없이 직접 신청할 수 있다. 다만 자동 확인은 운영 권한의 최종 보증이 아니라 위험을 분류하는 수단이다. 사업자 정상 여부, 표준화한 사업장 주소와 테니스장 장소 정보의 일치, 이미 승인된 동일 장소 운영자 부재를 모두 충족한 경우에만 공개 권한을 자동 부여한다. 이외의 불일치·공공시설·지점·위탁운영·중복 장소 사례는 증빙과 운영 검토를 거쳐 공개한다. 자동 확인이 끝난 운영자는 비공개 코트·시간대 초안을 만들 수 있지만, 공개 권한이 없으면 코트 매칭으로 공개할 수 없다.
...
공개한 제휴 코트 시간은 코트 면·시각·비용·현장 정원·이용 안내를 직접 바꿀 수 없는 공개 기록으로 취급한다. 오류가 발견되면 운영자는 그 시간을 `공개 중지`하고 새 비공개 초안을 등록한다. 이미 공개한 시간은 일반 수정이나 임의 중지가 불가능하며, 실제 공급 불가·시설 폐쇄·안전 문제 같은 긴급 사유만 감사 이력과 함께 매칭 취소·인앱 안내로 처리한다. 운영자 귀책으로 공개한 코트 매칭을 반복 철회하면 새 시간 공개를 자동 일시 중지하고 운영 검토 후에만 해제한다. 이 정책은 일반 사용자가 운영자에게 코트 예약을 요청하는 권한을 만들지 않는다.

### 14.2 Court Commerce 확장 원칙

> **주의:** 이 절은 일반 모집자가 코트 이용 총액을 결제하던 옛 모델을 전제로 쓰였다. 코트 매칭의 주최자가 운영자로 바뀌면서 결제 주체와 환불 대상이 달라지므로, Commerce를 시작하기 전에 다시 정해야 한다. 아래 수수료 정책은 사용자가 확정한 값이라 그대로 둔다.
